...
- Endpoints implementados: `GET /api/health` y CRUD completo (mismo patrón `GET/POST /api/campaigns/{id}/<entidad>` + `GET/PUT/DELETE /api/<entidad>/{id}`) para `campaigns`, `arcs`, `locations`, `npcs`, `player-characters`, `quests`, `sessions` y `groups`. También `GET /api/groups/:id/members` (JOIN `npc_groups`+`npcs`, primer query no-1:1-a-struct del proyecto). Pendientes: relaciones many-to-many sueltas (`quest_npcs`, `session_npcs`, etc — sin endpoint definido aún), `dashboard`, `notes/render`, `admin/reindex`, y el indexador del vault (ver `docs/VAULT_INDEXER.md`).
