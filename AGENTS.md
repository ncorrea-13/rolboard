...
- Endpoints implementados: `GET /api/health`, CRUD completo de `campaigns` (`GET /api/campaigns`, `POST /api/campaigns`, `GET/PUT/DELETE /api/campaigns/{id}`) y de `arcs` (`GET/POST /api/campaigns/{id}/arcs`, `GET/PUT/DELETE /api/arcs/{id}`). Resto de las entidades (`npcs`, `locations`, `groups`, `player-characters`, `quests`, `sessions`) siguen pendientes — ver `docs/API.md`.
