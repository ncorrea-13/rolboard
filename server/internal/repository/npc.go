...
func (r *NPCRepository) Create(ctx context.Context, n *models.NPC) error {
	...
	err := r.db.QueryRowContext(ctx, `
		INSERT INTO npcs (campaign_id, name, npc_kind, detail_level, status, location_id, etnia, rol, tipo_spren, description, notes, attributes, skills, obsidian_path)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		...
		RETURNING id, campaign_id, name, npc_kind, detail_level, status, location_id, etnia, rol, tipo_spren, description, notes, attributes, skills, obsidian_path, created_at, updated_at`,
		n.CampaignID, n.Name, n.NPCKind, n.DetailLevel, n.Status, toNullInt64(n.LocationID), toNullString(n.Etnia), toNullString(n.Rol), toNullString(n.TipoSpren), n.Description, n.Notes, toJSONText(n.Attributes), toJSONText(n.Skills), toNullString(n.ObsidianPath),
...
func (r *NPCRepository) Update(ctx context.Context, id int64, n *models.NPC) error {
	...
	err := r.db.QueryRowContext(ctx, `
		UPDATE npcs
		SET name = ?, npc_kind = ?, detail_level = ?, status = ?, location_id = ?, etnia = ?, rol = ?, tipo_spren = ?, description = ?, notes = ?, attributes = ?, skills = ?, obsidian_path = ?, updated_at = datetime('now')
		WHERE id = ? AND deleted_at IS NULL
		RETURNING id, campaign_id, name, npc_kind, detail_level, status, location_id, etnia, rol, tipo_spren, description, notes, attributes, skills, obsidian_path, created_at, updated_at`,
		n.Name, n.NPCKind, n.DetailLevel, n.Status, toNullInt64(n.LocationID), toNullString(n.Etnia), toNullString(n.Rol), toNullString(n.TipoSpren), n.Description, n.Notes, toJSONText(n.Attributes), toJSONText(n.Skills), toNullString(n.ObsidianPath), id,
	).Scan(&n.ID, &n.CampaignID, &n.Name, &n.NPCKind, &n.DetailLevel, &n.Status, &locationID, &etnia, &rol, &tipoSpren, &n.Description, &n.Notes, &attributes, &skills, &obsidianPath, &n.CreatedAt, &n.UpdatedAt)
	if err == sql.ErrNoRows {
		return ErrNotFound
	}
