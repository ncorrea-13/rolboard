...
func scanEncounterParticipant(row interface {
	...
	err := row.Scan(&p.ID, &p.EncounterID, &pcID, &npcID, &displayName, &currentHp, &maxHp,
		&initiative, &turnType, &p.Notes, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		return err
	}
...
func (r *EncounterParticipantRepository) List(ctx context.Context, encounterID int64) ([]models.EncounterParticipant, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, encounter_id, pc_id, npc_id, display_name, current_hp, max_hp, initiative_value, turn_type, notes, created_at, updated_at
...
func (r *EncounterParticipantRepository) Create(ctx context.Context, p *models.EncounterParticipant) error {
	row := r.db.QueryRowContext(ctx, `
		INSERT INTO encounter_participants (encounter_id, pc_id, npc_id, display_name, current_hp, max_hp, initiative_value, turn_type, notes)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
		RETURNING id, encounter_id, pc_id, npc_id, display_name, current_hp, max_hp, initiative_value, turn_type, notes, created_at, updated_at`,
...
func (r *EncounterParticipantRepository) Update(ctx context.Context, id int64, p *models.EncounterParticipant) error {
	row := r.db.QueryRowContext(ctx, `
		...
		SET pc_id = ?, npc_id = ?, display_name = ?, current_hp = ?, max_hp = ?,
		    initiative_value = ?, turn_type = ?, notes = ?, updated_at = datetime('now')
		WHERE id = ? AND deleted_at IS NULL
		RETURNING id, encounter_id, pc_id, npc_id, display_name, current_hp, max_hp, initiative_value, turn_type, notes, created_at, updated_at`,
		toNullInt64(p.PcID), toNullInt64(p.NpcID), toNullString(p.DisplayName),
		toNullInt64(p.CurrentHp), toNullInt64(p.MaxHp), toNullInt64(p.Initiative), toNullString(p.TurnType), p.Notes, id,
	)
	err := scanEncounterParticipant(row, p)
	if err == sql.ErrNoRows {
		return ErrNotFound
