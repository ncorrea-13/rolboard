...
export function ArcsList({ arcs, onSelect }: { arcs: Arc[]; onSelect: (id: string) => void }) {
  return (
    <div className="card list-page">
      <div className="list-page__header">
        <div className="display" style={{ fontSize: 21 }}>Arcos</div>
        <span className="list-page__count">{arcs.length} arcos</span>
      </div>
      <div className="list-page__rows">
        {arcs.map((a) => (
          <div key={a.id} className="list-page__row list-page__row--clickable" onClick={() => onSelect(a.id)}>
            ...
            <StatusPill status={a.status} label={a.status === "alive" ? "En curso" : "Cerrado"} />
          </div>
        ))}
      </div>
    </div>
