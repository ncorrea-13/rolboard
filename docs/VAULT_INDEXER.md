...
| Key                 | Use |
| ------------------- | --- |
| `tipo`              | `npc_kind`: the key of one of the campaign's NPC types (`npc`, `spren`, `entidad-cognitiva` by default; manage them in the app) or `referencia`. Any other value → error on that note |
